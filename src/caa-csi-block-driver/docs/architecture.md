...
- **Availability Zone (AWS):** EBS volumes can only be attached to instances
  in the **same Availability Zone**. Ensure your StorageClass
  `awsAvailabilityZone` matches the AZ where worker nodes (and PodVMs) run.
  Alternatively, use `volumeBindingMode: WaitForFirstConsumer` with topology
  keys to let Kubernetes schedule the volume in the correct zone.

- **Region (Azure):** Managed Disks must be in the same region as the PodVM.
  The CAA Azure provider validates this at creation time and rejects mismatches.

- **hostPath mount (CAA DaemonSet):** The cloud-api-adaptor DaemonSet must
  mount `/run/kata-containers/shared/direct-volumes` from the host so it can
  read the `mountInfo.json` files written by this CSI driver. This is included
  in the peerpods Helm chart by default.
...
### Solution
The `peerpod-ctrl` component handles cleanup of orphaned PodVMs. When stale
VMs are deleted, their attached disks are automatically released by the cloud
provider. The CAA provider validates disk state before VM creation — if a disk
is still attached, creation fails immediately with a clear error rather than
...
2. **Per-disk validation** (`validateDisks`): for each volume, verifies:
