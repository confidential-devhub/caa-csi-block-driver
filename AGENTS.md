...
- The csi-sanity skip list does **not** mean "unimplemented": snapshot, expand, clone, and stats are implemented but skipped because the harness can't exercise them against real cloud disks. Genuinely unimplemented: `ControllerPublish/UnpublishVolume` (by design — CAA attaches disks via the cloud API, not CSI publish), `ListVolumes`, `GetCapacity`, `ModifyVolume`, `GroupController`. Keep the skip list in `hack/run-csi-sanity.sh` in sync with `.github/workflows/csi-sanity.yaml` when touching RPC scope.
