...
// CSI tag and rebuilds any missing local volume records.
...
func (vs *volumeStore) listAll() ([]*volumeRecord, error) {
	...
	for _, e := range entries {
		if filepath.Ext(e.Name()) != ".json" || e.Name() == "_manifest.json" {
			continue
		}
		data, err := os.ReadFile(filepath.Join(vs.dir, e.Name()))
		if err != nil {
...
// the cloud provider params as a lightweight backup for recovery.
